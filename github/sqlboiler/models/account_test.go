...
func testAccountsExists(t *testing.T) {
	...
	e, err := AccountExists(tx, o.Userid)
	if err != nil {
...
func testAccountsFind(t *testing.T) {
	...
	accountFound, err := FindAccount(tx, o.Userid)
	if err != nil {
		t.Error(err)
	}
...
var (
	accountDBTypes = map[string]string{`Account`: `varchar`, `CreatedAt`: `timestamp`, `Nickname`: `varchar`, `Userid`: `bigint`}
	_              = bytes.MinRead
)
